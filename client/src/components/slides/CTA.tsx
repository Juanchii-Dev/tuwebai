...
export default function CTA({ isActive }: { isActive: boolean }) {
  ...
  // Si la diapositiva no está activa, volver automáticamente a opciones
  useEffect(() => {
    if (!isActive) {
  ...
  return (
    <Slide id="cta" isActive={isActive} withGradient>
      <Blob color="accent" className="w-96 h-96 left-1/3 top-1/3 opacity-30" />
      <Blob color="secondary" className="w-80 h-80 right-1/3 bottom-1/3 opacity-30" />
            ...
            {activeTab === "options" ? (
              ...
              >
                {ctaButtons.map((button, index) => (
                  ...
                  >
                    <CardHover hoverColor={button.color as "secondary" | "accent"} className="text-center h-full cursor-pointer">
              ...
              >
                ...
                <div className="contacto-wrapper h-full">
                  <ContactForm />
                </div>
              </motion.div>
            )}
